...
public class EigenvectorsActivity extends AppCompatActivity {
    int row = 0;
    int col = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_eigenvectors);

        Spinner spinnerDim = (Spinner) findViewById(R.id.spinner_eigenvectors);
        Button btn_calc = (Button) findViewById(R.id.calc_eigenvectors);
        final TextView result = (TextView) findViewById(R.id.textView_eigenvectors_result);
        ...
        btn_calc.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                GridView grid_eigenvalues = (GridView)findViewById(R.id.grid_eigenvectors);
                ...
                Jama.Matrix eigenVec = eigen.getV();

                // print eigen vector
                eigenVec.print(8 ,5 );
                // System.out.println(eigenVec.get(1,2));

                // Set text to show result
                result.setText("");
                for(int i = 0; i < realPart.length; i++){
                    double [][] eigenVec_temp = new double[1][realPart.length];
                    result.append("Eigenvector " + i + " :\n[ ");
                    for(int j = 0; j < realPart.length; j++){
                        eigenVec_temp[0][j] = eigenVec.get(j, i);
                        //System.out.print(eigenVec_temp[0][j] + "\t\t");
                        result.append(df.format(eigenVec_temp[0][j]));
                        if(j != realPart.length-1){
                            result.append(" , ");
                        } else {
                            result.append(" ");
                        }
    ...
    private void setMatrix() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                GridView grid = (GridView)findViewById(R.id.grid_eigenvectors);
